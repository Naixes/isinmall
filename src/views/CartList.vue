<template>
  <div>
              ...
              <li v-for="item in cartList" :key="item.productId">
                ...
                <div class="cart-tab-5">
                  <div class="cart-item-opration">
                    <a class="item-edit-btn" @click="delConfirm(item)">
          ...
          <div class="cart-foot-inner">
            <div class="cart-foot-l">
              <div class="item-all-check">
                <a>
                  <span class="checkbox-btn item-check-btn" :class="{check:checkAll}" @click="checkAllProd(cartList)">
    ...
    <modal :mdShow="delConfirmShow">
      <p slot="message">确认删除该商品？</p>
      <div slot="btnGroup">
        <a class="btn btn--m" @click="delProductById">确认</a>
        <a class="btn btn--m" @click="delConfirmShow=false">取消</a>
      </div>
    </modal>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      totalPrice: {
        get() {
          let tatal = 0
          this.cartList.forEach(item => {
            if(item.checked == '1' ? true : false) tatal += item.salePrice*item.productNum
          })
          return tatal
        },
        set() {}
    ...
    methods: {
      ...
      delProductById() {
        axios.post('/users/cartDel', {productId: this.delItem.productId}).then(result => {
          let res = result.data
          if(res.status == '0') {
      ...
      cartEdit(flag, item) {
        ...
        axios.post('users/cartEdit', {productId: item.productId, productNum: item.productNum, checked: item.checked}).then(result => {
          let res = result.data
          if(res.status == '0') {
      ...
      // 选中所有商品
      checkAllProd(cartList) {
        let flag = !this.checkAll
        cartList.map(item => {
            item.checked = flag ? 1 : 0
        })

        axios.post('/users/checkAll', {checkAll: flag}).then(result => {
          let res = result.data
          if(res.status == '0') {
