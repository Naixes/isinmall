<template>
  <div>
      ...
      <div class="container">
        <div class="checkout-addr">
              ...
              <ul>
                <li v-for="(item,index) in addressListLimit" :key="item.addressId" :class="{check:index==checkIndex}" @click="checkIndex=index,checkAddressId=item.addressId">
                  ...
                  <div class="addr-opration addr-del">
                    <a class="addr-del-btn" @click="delConfirmShow=true">
          ...
          <div class="next-btn-wrap">
            <router-link :to="{path:'/orderconfirm',query:{addressId:checkAddressId}}" class="btn btn--m btn--red">Next</router-link>
          </div>
        </div>
      </div>
    </div>
    <modal :mdShow="delConfirmShow">
      ...
      <div slot="btnGroup">
        <a class="btn btn--m" @click="delAddress(checkAddressId)">确认</a>
  ...
  export default {
    ...
    methods: {
      getAddressList() {
        axios.get('/users/address').then(result => {
          ...
          if(res.status == '0') {
            ...
            // 初始化选中地址为默认地址，没有点击选择时将默认地址传入下一个页面
            this.checkAddressId = this.addressList.find(item => item.isDefault).addressId || ''
          }
        })
      },
      more() {
      ...
      delAddress(addressId) {
        axios.post('/users/delAddress', {addressId}).then(result => {
          let res = result.data
          if(res.status == '0') {
